#pythran export next_perm_pythran(int list)
def next_perm_pythran(l):
    """
...
#pythran export num_descents_pythran(int list)
def num_descents_pythran(l):
    s = 0
    for i in range(len(l)-1):
        s += l[i+1] < l[i]
    return s

#pythran export count_perm_by_descents_pythran(int)
def count_perm_by_descents_pythran(n):
    res = [0] * n
    l = list(range(n))
    res[num_descents_pythran(l)] += 1
    while next_perm_pythran(l):
        res[num_descents_pythran(l)] += 1
    return res
